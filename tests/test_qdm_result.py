...
class TestMagneticMap:
    ...
    def test_magnetic_map_uses_settings_nv_axis_by_default(
        self, qdm_result: QDMResult
    ) -> None:
    ...
    def test_magnetic_map_passes_custom_nv_axis(
        self, two_pol_fit_result: FitResult
    ) -> None:
        axis = (0.1, 0.8, 0.6)
        result = QDMResult(fit_result=two_pol_fit_result, nv_axis=axis)
        with patch('QDMpy.magnetic_map.MagneticMap.from_b111') as mock_from_b111:
...
class TestPersistence:
    ...
    def test_roundtrip_no_nv_axis(
        self, qdm_result: QDMResult, tmp_path: Path
    ) -> None:
        out = tmp_path / 'result'
        qdm_result.save(out)
    ...
    def test_roundtrip_with_nv_axis(
        self, two_pol_fit_result: FitResult, tmp_path: Path
    ) -> None:
        axis = (0.0, 0.8164966, 0.5773503)
    ...
    def test_roundtrip_b111_remanent(
        self, qdm_result: QDMResult, tmp_path: Path
    ) -> None:
        out = tmp_path / 'result'
        qdm_result.save(out)
