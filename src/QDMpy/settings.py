...
class FitSettings(BaseModel):
    ...
    estimator: Literal['LSE', 'MLE'] = Field(
        default='MLE', description='Estimator type'
    )
    max_number_iterations: int = Field(
        default=1000, description='Maximum iterations for fitting'
    )
...
class StatisticsPercentileSettings(BaseModel):
    ...
    width_percentile: list[float] = Field(
        default=[0, 99.0], description='Width percentile bounds'
    )
...
class LocalOutlierFactorSettings(BaseModel):
    ...
    contamination: str | float = Field(
        default='auto', description='Expected outlier fraction'
    )
...
class QDMpySettings(BaseSettings):
    ...
    odmr: OdmrSettings = Field(
        default_factory=OdmrSettings, description='ODMR settings'
    )
    model: ModelSettings = Field(
        default_factory=ModelSettings, description='Model settings'
    )
    fit: FitSettings = Field(
        default_factory=FitSettings, description='Fitting settings'
    )
    ...
    def settings_customise_sources(
        ...
        toml_settings = (
            TomlConfigSettingsSource(settings_cls, config_file)
            if config_file.exists()
            else None
        )

        return tuple(
            s for s in [init_settings, env_settings, toml_settings]
            if s is not None
        )
