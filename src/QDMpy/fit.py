...
- Model selection: Automatic or manual selection of appropriate spectral models (ESR14N, ESR15N, ESRSINGLE)
...
class Fit:
    ...
    def set_constraints(
        ...
        # Handle special case for contrast parameters
        if param == "contrast" and self.model_params_unique != self.model_params:
            for contrast_param in [p for p in self.model_params_unique if "contrast" in p]:
                self.set_constraints(
                    contrast_param, vmin=vmin, vmax=vmax, constraint_type=constraint_type, reset_fit=False
                )
        else:
    ...
    def set_free_constraints(self) -> None:
        ...
        """
        for param in self.model_params_unique:
            self.set_constraints(param, constraint_type="FREE", reset_fit=False)
        self._reset_fit()
    ...
    def get_initial_parameter(self) -> NDArray:
        ...
        # Generate guesses for each parameter type
        for param_type in set(p.split('_')[0] for p in self.model_params_unique):
            ...
            if param_type == "center":
                ...
                param_values = np.ones((n_pol, n_frange, n_pixel)) * 0.0
            else:
                raise ValueError(f"Unknown parameter type: {param_type}")
                
            # Add parameter for each instance in the model
            for i, param in enumerate([p for p in self.model_params_unique if p.split('_')[0] == param_type]):
                fit_parameters.append(param_values)
        
        # Stack parameters along the last axis
        return np.ascontiguousarray(np.stack(fit_parameters, axis=-1), dtype=np.float32)
