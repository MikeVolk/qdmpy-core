...
def plot_folding_search_landscape(folded: FoldedODMR) -> None:
    ...
    for i_pol in range(n_pol):
        ...
        ax.axvline(
            d_cand[best_idx], color='tab:red', ls='--', lw=1.5,
...
def plot_folding_overview(folded: FoldedODMR) -> None:
    ...
    if folded.d_candidates is not None and folded.search_residual is not None:
        ...
        for i in range(2):
            axes[0, i].text(
                0.5, 0.5, 'No search data', transform=axes[0, i].transAxes,
                ha='center', va='center',
            )
    ...
    im0 = axes[1, 0].imshow(
        (d_mean - D_ZFS) * 1000, cmap='RdBu_r', origin='upper', aspect='equal',
    )
    ...
    im1 = axes[1, 1].imshow(
        res_mean, cmap='magma', origin='upper', aspect='equal', vmin=0, vmax=1,
    )
